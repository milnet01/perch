...
`state.json` has its own registry, `STATE_MIGRATIONS` in [`src/perch/core/state_store.py`](../src/perch/core/state_store.py), wired and empty on the same terms. Its failure mode differs because losing remembered geometry is not fatal: an unmigratable or too-new document leaves the store empty and **latched read-only**, so nothing is written back and the file survives a Perch that cannot read it.
