...
2. If multiple windows would take the same entry (two Firefox windows), the geometry goes to **the focused one** if it is among them, otherwise to the first of them in the backend's window list. The others are left alone for this pass — not moved, not restored. "Focused" is `WindowBackend.get_active_window()` asked once per pass; a backend that cannot report focus (it raises `BackendUnsupported` — today every backend but KWin) gets the first-match choice. Perch keeps no focus history, so "most recently used" is not an option. A window that a `[[rules]]` entry claims first never competes for a layout entry.
...
- [`src/perch/core/layouts.py`](../src/perch/core/layouts.py) — `Layout` / `LayoutEntry` dataclasses, `parse_layouts(raw)`. The engine emits an `ApplyActionDecision` per layout-matched window, taking the **last** entry that matches it (`engine.layout_entry_index`). The reducer's apply pass (`Reducer._evaluate_all_open_windows`) groups the windows whose decision came from the layout by that entry and keeps one per group, per §Apply semantics step 2.
- Entries skipped for an absent output are collected across one apply pass by `Reducer._evaluate_all_open_windows` and handed to the `notify_skipped` callback that the composition root wires to the tray balloon ([`src/perch/ui/status.py`](../src/perch/ui/status.py)). The pass is the unit — a lone window event logs and does not notify — and the reducer builds no user-facing string, so wording and translation stay in the UI.
