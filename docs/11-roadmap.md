...
| 4 | Implementation | in progress (M1 + M2 + M2.5 done; M3 in progress — M3.a + M3.b + M3.d landed) | Milestones M1…M9 below, with an injected M2.5 spike |
...
**Status:** in progress. M3.a (tray skeleton + intents + SNI probe + app wiring), M3.b (config dialog scaffold + General/Rules/Exclusions edit flows via tomlkit in-place mutation + `RulesModel` with `InternalMove` drag-reorder) and M3.d (i18n plumbing — `translations/` with `.ts` extraction via `pyside6-lupdate`, `i18n.py` loader) landed 2026-04-20. M3.c (reusable widgets: match-editor, geometry-editor, key-capture) and M3.e (screenshots + AppStream metainfo) are still pending.
