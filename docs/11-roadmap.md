...
**Moved.** Live and planned work now lives in [`ROADMAP.md`](../ROADMAP.md) at
the repository root, where each item carries a `PERC-NNNN` id and is tracked in
the roadmap store. The release themes are unchanged: v1.0.1 (get it
downloadable), v1.1 (onboarding & robustness), v1.2 (smarts), v2.0
(Wayland-native), the Windows edition track, and someday / unscheduled.

This file remains the source of truth for milestone ordering, the ground rules,
the known-risks list and the Phase 2 / 2.5 research logs.
