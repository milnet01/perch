...
2. **Health checks** — three rows, each rendered ✓ (ok) or ⚠ (needs attention)
   ...
   | Check | Source | ⚠ state |
   |---|---|---|
   | **Tray icon visible** | `sni_probe.sni_host_available()` | On GNOME Wayland (`sni_probe.is_gnome_wayland()`), shows the AppIndicator install guidance + link (the same copy as the standalone hint). On any other desktop: *"couldn't confirm — Perch still runs; reach it with `perch --settings`."* |
   | **Start Perch at login** | `config.general.start_at_login` (the persisted intent that governs autostart via `autostart.sync_from_config()` at startup) | A live checkbox initialised from that stored value — so it reads ✓ on a fresh install (the field defaults on). Toggling records the new desired state, applied via `autostart.sync()` when the wizard is dismissed. The badge is driven by the stored intent, **not** by `autostart.is_enabled()`, because on the Flatpak/portal path `is_enabled()` always returns `False` (it can't cheaply query portal autostart) and would show a false ⚠. |
   | **Compositor detected** | the backend selected at startup | Backends expose no display name, so the wizard owns a small `type(backend) → label` map in `onboarding.py` (KWin/Plasma, X11, Sway, …). ⚠ *"limited mode"* when Perch fell back to `MockBackend` (no compatible compositor). |
...
- At startup (`src/perch/app.py`), if `not config.general.onboarding_completed`
  the wizard runs modally. Because the Compositor row needs the selected
  backend, backend selection (`_select_backend()`) must move ahead of the wizard
  call — today it happens *after* the point where `_maybe_show_appindicator_hint`
  fires, so the implementer reorders startup to select the backend first, then
  run the wizard in the hint's place.
- On **any** dismissal — **Finish** or the window close button — Perch persists
  `onboarding_completed = true` (and any autostart toggle) via `apply_general`
  in `src/perch/config/edit.py`. That writer takes the whole `[general]` block,
  so the wizard loads the current config and passes every field plus the new
  flag (add `onboarding_completed` to `apply_general` and `_GENERAL_BOOL_KEYS`).
  It never reappears unprompted.
- On the launch the wizard runs, Perch **skips** the standalone
  `_maybe_show_appindicator_hint` — the tray health-check row already covers the
  no-tray case, so there is no double notification. Once onboarding is complete,
  that recurring GNOME-Wayland hint behaves as before (the safety net for a
