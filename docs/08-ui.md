...
2. **Health checks** — three rows, each rendered ✓ (ok) or ⚠ (needs attention)
   ...
   | **Start Perch at login** | `autostart.is_enabled()` | A live checkbox; toggling it records the desired state, applied via the existing `autostart.sync()` path when the wizard is dismissed. Defaults on. |
   | **Compositor detected** | the backend selected at startup | Shows the picked backend's friendly name (KWin / Plasma, X11, Sway, …). ⚠ *"limited mode"* when Perch fell back to `MockBackend` (no compatible compositor). |
...
- At startup (`src/perch/app.py`), if `not config.general.onboarding_completed`
  the wizard runs modally. On **any** dismissal — **Finish** or the window close
  button — Perch persists `onboarding_completed = true` (and any autostart
  toggle) through the `src/perch/config/edit.py` writer, so it never reappears
  unprompted.
...
button (beside *Open config folder*). It calls the same
