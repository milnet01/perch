...
def evaluate(
    ...
    *,
    ...
    if active_layout is not None:
        ...
        index = layout_entry_index(active_layout, window)
        if index is not None:
            return ApplyActionDecision(
                action=active_layout.windows[index].apply,
...
def layout_entry_index(layout: Layout, window: WindowInfo) -> int | None:
    """Index of the entry of ``layout`` that ``window`` takes — the last match."""
    chosen: int | None = None
    for i, entry in enumerate(layout.windows):
